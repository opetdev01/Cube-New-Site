  ...
  return (
    <div ref={containerRef} className={styles.wrapper}>
      ...
      <section className={styles.section}>
        <span className={`${styles.label} reveal`}>Global Metrics</span>
        <h2 className={`${styles.title} reveal`}>Cube <br /> <span style={{color: '#e30613'}}>Consultants</span> <br /> In Numbers</h2>
        ...
        <div className={`${styles.yearsBadge} reveal`}>
           ...
           <div className={styles.metricLabel}>Years of <br /> Engineering <br /> Mastery</div>
        </div>
      </section>

      {/* 2. GLOBAL FOOTPRINT SECTION */}
      <section className={`${styles.section} ${styles.mapSection}`}>
        <span className={`${styles.label} reveal`}>Worldwide Network</span>
        <h2 className={`${styles.title} reveal`}>Cube <br /> Globally</h2>
      ...
      <section className={styles.section} style={{ paddingBottom: 0 }}>
        <span className={`${styles.label} reveal`}>The Moral Principles</span>
        <div className={styles.principlesGrid}>
          {IdentityData.principles.map((p, i) => (
            <div key={i} className={`${styles.principleItem} reveal`}>
              <span className={styles.pHeader}>{p.id}. {p.title}</span>
              <h3 className={styles.pTitle}>{p.core}</h3>
              <ul className={styles.pList}>
                {p.details.map((d, di) => <li key={di}>{d}</li>)}
              </ul>
              <div className={styles.pCommitment}>Commitment: {p.commitment}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className={`${styles.section} ${styles.starSection}`}>
        <span className={`${styles.label} reveal`}>2026 Design Philosophy</span>
        ...
        <h2 className="reveal" style={{ fontSize: '1.5rem', fontWeight: 900, textTransform: 'uppercase' }}>
            Spaces that heal, inspire, and connect
        </h2>
      </section>

      {/* 5. CORE DEPARTMENTS SECTION */}
      <section className={styles.section}>
        <span className={`${styles.label} reveal`}>Divisional Excellence</span>
        <div className={styles.servicesGrid}>
          {IdentityData.services.map((s, i) => (
            <div key={i} className={`${styles.serviceItem} reveal`}>
              <span className={styles.label} style={{ marginBottom: '20px' }}>Part {s.id}</span>
              <h3 className={styles.sTitle}>{s.name}</h3>
              <ul className={styles.pList} style={{ marginTop: '30px' }}>
                {s.bullets.map((b, bi) => <li key={bi}>{b}</li>)}
              </ul>
              <div className={styles.sValue}>Value: {s.value}</div>
            </div>
          ))}
        </div>
      </section>
